...
import {WrappedZlwnft} from "./WrappedZlwnft.sol";
...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    (
        ...
        wnft.transferFrom(address(this), oldOwner, tokenId);
        wnft.burn(tokenId);

        //construct data to be sent to the destination chain
        bytes memory payload = abi.encode(tokenId, oldOwner);

        bytes32 messageId =  sendMessagePayLINK(chainSelector,_receiver,payload);
